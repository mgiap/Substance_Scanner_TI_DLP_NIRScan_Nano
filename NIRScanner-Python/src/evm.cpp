...
EVM::EVM()
{
    m_RefCalMatrixBlob = (unsigned char *)malloc(REF_CAL_MATRIX_BLOB_SIZE);
    m_RefCalDataBlob = (unsigned char *)malloc(SCAN_DATA_BLOB_SIZE);
}
	 ...
	 */
{
    ...
    if(ret == DLPSPEC_PASS)
    {
        ...
        std::cout << "dlpspec_get_scan_config_dump_size failed." << std::endl;
    }
 ...
 */
{
    ...
    string refCalMatrixFileName = "refCalMatrix_";
    string refCalMatrixFileNameFull;
...
//        file.close();
        std::cout << "RefCalMatrix fetched " << refCalSize << std::endl;
        return PASS;
    }
    else
    {
        std::cout << "RefCalMatrix failed." << std::endl;
        return FAIL;
    }
}
