...
"""NIRScanner Python wrapper (cleaned).

This file provides a lightweight Python wrapper around the native `_NIRScanner` Python
extension compiled from the original project. The wrapper exposes a `NIRS` class for
configuring and acquiring scans from the DLP NIRScan Nano.

If the compiled extension is not present, importing this module will fail — callers
should handle that scenario (see `scan_substance.py` which falls back to simulated data).
"""
...
class NIRS:
    ...
    def _cleanup(self):
        print("Cleaning up NIRS instance.")
        delete_NIRScanner(self.nirs_obj)
    ...
    def scan_snr(self, scan_type="hadamard"):
        if scan_type == "hadamard":
            ...
            print("Unknown scan type {}.".format(scan_type))
        results_str = NIRScanner_scanSNR(self.nirs_obj, hadamard_flag)
